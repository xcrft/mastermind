...
| `mmcg_map` | Schema-v1 project map with lexical scope (`%`/`_` literal), directory-relative components, `depth` 1–6, `top` 1–100, optional `production_only`, and caps of 50,000 paths, 20 languages, 20 components, 20 boundaries/component and 400 globally, 50 entry points, 100 hotspots, 50,000 cycle edges, 50 cycles, and 500 memberships. The production filter removes conventional test, fixture, example, demo, benchmark, generated, vendor, dependency, and build-target paths before bounded queries run. `path_work_limit` marks partial path aggregates; `top_probe` marks hotspot and per-component boundary cap+1 probes; `global_probe_limit` marks uncertainty caused by the 401st global boundary row; cycle `work_limit` skips SCC analysis. |
