...
impl Store {
    ...
    pub fn commit_file(&mut self, pending: PendingFile) -> SqlResult<()> {
        ...
        let mut symbol_ids: Vec<i64> = Vec::with_capacity(pending.symbols.len());
        {
            let mut stmt = tx.prepare_cached(
                "INSERT INTO symbols(name, kind, file_path, line_start, line_end, signature, parent_id, language)
    ...
    /// Excludes synthetic `<module>` rows (never "called" by definition).
    ///
    /// **Honest false-positives** (caller responsibility to filter):
    /// - Entry points (`main`, framework-registered handlers, exports consumed by tooling)
    /// - Dynamic dispatch / reflection / trait objects whose calls don't surface as `to_name = method`
    /// - Cross-language calls (TS → Python subprocess; neither end's binding shows here)
    ///
