...
Where `<slug>` names the workflow the template wires up (e.g. `mastermind-workflow`, `mastermind-context`).

### Frontmatter
```yaml
---
name: mastermind-workflow
description: CLAUDE.md that pre-wires the planner+executor delegation workflow — drop-in setup for projects using `.mastermind/tasks/` specs.
metadata:
  version: 0.1.0
  tags:
    - claude-md
