...
| [`mcp/`](mcp/) | MCP server configs. Headlined by **`mmcg`** — fast Rust codegraph indexer for Python/TS/JS/Rust with 15 structural query tools. |
...
Underneath every role sits **mmcg** — a Rust binary that indexes Python, TypeScript, JavaScript, Rust, C#, Go, Java, PHP, and C/C++ into a local SQLite codegraph and exposes 15 structural query tools over MCP (`search`, `callers`, `callees`, `impact`, `imports`, `imported_by`, `files`, `symbols_in_file`, `outline`, `recent_changes`, `unreferenced`, `api_surface`, `centrality`, `tasks`, `dependency_cycles`). Every subagent queries mmcg before claiming anything about the code: "does function X exist?", "who calls Y?", "transitive blast radius of changing Z?", "what past specs touched this area?", "any new import cycles?". This is what makes the workflow's claims resistant to hallucination.
...
`mmcg` is a standalone [crate on crates.io](https://crates.io/crates/mmcg) — 15 structural query tools (search, callers, callees, impact, outline, recent_changes, unreferenced, api_surface, centrality, tasks, dependency_cycles, …) for Python / TypeScript / JavaScript / Rust / C# / Go / Java / PHP / C/C++. Zero system deps, sub-millisecond queries.
