...
use super::common::{line_of, node_text, push_call, push_call_with_type, push_def, push_import};
    ...
    for child in node.children(&mut cursor) {
        match child.kind() {
            ...
            "struct_item" => {
                ...
                push_def(pending, name, "struct", &child, sig, parent_index);
            }
            "enum_item" => {
                let name = name_field(&child, source).unwrap_or("<anon>").to_string();
                let sig = signature_until_body_or_semi(&child, source);
                push_def(pending, name, "enum", &child, sig, parent_index);
            }
            "trait_item" => {
                let name = name_field(&child, source).unwrap_or("<anon>").to_string();
                let sig = signature_until_body_or_semi(&child, source);
                let idx = push_def(pending, name, "trait", &child, sig, parent_index);
                if let Some(body) = child.child_by_field_name("body") {
                    walk(body, source, pending, Some(idx), module_index);
                }
            }
            "impl_item" => {
                ...
                let sig = signature_until_body_or_semi(&child, source);
                let idx = push_def(pending, target_name, "impl", &child, sig, parent_index);
                if let Some(body) = child.child_by_field_name("body") {
                    walk(body, source, pending, Some(idx), module_index);
                }
            }
            "mod_item" => {
                ...
                let name = name_field(&child, source).unwrap_or("<anon>").to_string();
                let sig = signature_until_body_or_semi(&child, source);
                let idx = push_def(pending, name, "mod", &child, sig, parent_index);
                if let Some(body) = child.child_by_field_name("body") {
                    walk(body, source, pending, Some(idx), module_index);
                }
            }
