...
| `mmcg_files` | optional `prefix`, `language`, `top` (default 200, max 500) | Indexed files with symbol counts. MCP responses fetch at most `top` rows and expose `truncated` plus `row_limit`; narrow by literal prefix or language when truncated. The CLI file query retains its complete local listing. |
...
| `mmcg_api_surface` | `prefix`, optional `language`, `top` (default 100, max 500) | Symbols under the literal `prefix` referenced from at least one file outside it. MCP responses expose the exact filtered `total`, returned `count`, `truncated`, and `row_limit`; narrow the prefix or add language when truncated. The CLI retains its complete local listing. Empirical "who-uses-this-module" map; doesn't need declared visibility. |
