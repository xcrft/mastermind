...
async function main() {
  ...
  const reverseRuntimePayload = fixture();
  reverseRuntimePayload.evidence.runtime_edges.items[0].parent_file = "src/target.rs";
  reverseRuntimePayload.evidence.runtime_edges.items[0].child_file = "src/auth.rs";
  const reverseRuntimeHarness = await renderFixture(reverseRuntimePayload, { width: 390 });
  reverseRuntimeHarness.nodes.get("mobile-trace-list").querySelectorAll(".mobile-candidate")[0].dispatch("click");
  const reverseRuntimeEdge = reverseRuntimeHarness.nodes.get("trace-graph").querySelectorAll("[data-edge-id]")[0];
  assert.ok(reverseRuntimeEdge, "A reverse runtime pair must remain inspectable as file-level evidence");
  assert.match(reverseRuntimeEdge.getAttribute("aria-label"), /runtime file pair observed in reverse direction/i);
  assert.doesNotMatch(reverseRuntimeEdge.getAttribute("aria-label"), /runtime trace corroborated/i);
  reverseRuntimeEdge.dispatch("click");
  const reverseRuntimeInspector = reverseRuntimeHarness.nodes.get("inspector-body").textContent;
  assert.match(reverseRuntimeInspector, /Runtime traceObserved in reverse direction/i);
  assert.match(reverseRuntimeInspector, /Runtime file-pair observation/i);
  assert.match(reverseRuntimeInspector, /src\/target\.rs → src\/auth\.rs/i);
  assert.doesNotMatch(reverseRuntimeInspector, /Runtime trace corroboration/i);
