...
- Three languages — extend by adding a parser, not by depending on multi-language toolchains
- 8 query tools that map directly to the workflow's needs
...
| `mmcg_callers` | `name`, optional `language`, `edge_kind` (default `calls`) | Symbols connected to `name` by the given edge kind. Use before editing for blast radius. Pass `edge_kind: imports` to find importers via the same tool. |
...
- **`mmcg_unreferenced` produces false positives.** A symbol with zero recorded callers / importers may still be live: (a) entry points like `main`, framework-registered HTTP handlers, public re-exports consumed by tooling; (b) dynamic dispatch — trait objects, Python duck-typed calls, JS reflection — where the call doesn't surface as `to_name = method`; (c) cross-language calls (TS subprocess invoking a Python script). Review every hit manually before deleting. The tool is "candidates to investigate", not "safe to delete".
...
This artifact is the **truth layer** for the [`mastermind-workflow`](../../../agents/claude-md/mastermind-workflow.md) — see that file for how planner/executor/researcher are taught to query mmcg first for code-structural questions. Integration wiring (skill updates that mandate codegraph-first lookups) lands in a follow-up round.
