...
| **Output** | 3 weaknesses + verdict (ship/caveats/revise/rethink) | Audit verdict (held/drift/broken) + discrepancy list |
...
Underneath every role sits the [`mmcg`](mcp/servers/mmcg/README.md) codegraph MCP — Python, TypeScript, and Rust code indexed into a SQLite graph with 8 structural query tools. Every role queries mmcg for code-structural questions:
...
This planning workflow is for **building things**. For **production breakage**, there's a separate parallel workflow: [`mastermind-incident-response`](../../skills/workflow/mastermind-incident-response/SKILL.md).
